// Cheng Yao @ 2013
// :-D -- Stolen from live555/liveMedia/RTSPClient.cpp
...
#define WFD_AUDIO_CODECS "LPCM 00000002 00"
////////// WFDSink implementation //////////

void afterSETUP(WFDSink* o, int resultCode, char* resultString) {
  o->startPlay();
}

ParsedMessage ParsedMessage::Parse(
...
ssize_t ParsedMessage::parse(const char *data, size_t size, bool noMoreData) {
    ...
    mapiter lastDictIter = mDict.end();
    ...
    while (offset < size) {
        ...
        if (line.c_str()[0] == ' ' || line.c_str()[0] == '\t') {
            ...
            //if (lastDictIndex >= 0) {
                ...
                //std::string &value = mDict.at(lastDictIndex);
                
                lastDictIter->second.append(line);
            }
        ...
        if (colonPos >= 0) {
            ...
            lastDictIter = mDict.find(key);
        }

        offset = lineEndOffset + 2;
    }

    for (mapiter i = mDict.begin(); i != mDict.end(); ++i) {
...
void ParsedMessage::getRequestField(size_t index, std::string &field) const {
    std::string line;
    //CHECK(findString("_", line));
    assert(findString("_", line));
...
WFDSink* WFDSink::createNew(UsageEnvironment& env, char const* rtspURL,
                  int verbosityLevel,
                  char const* applicationName,
                  portNumBits tunnelOverHTTPPortNum) {
  return new WFDSink(env, rtspURL,
            verbosityLevel, applicationName, tunnelOverHTTPPortNum);
}

unsigned WFDSink::sendDESCRIBECommand(responseHandler* responseHandler, Authenticator* authenticator) {
...
unsigned WFDSink::sendM2(responseHandler* responseHandler, Authenticator* authenticator) {
  if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "OPTIONS", responseHandler));
}

unsigned WFDSink::sendSETUPCommand(MediaSubsession& subsession, responseHandler* responseHandler,
...
unsigned WFDSink::sendPLAYCommand(MediaSession& session, responseHandler* responseHandler,
...
unsigned WFDSink::sendPLAYCommand(MediaSubsession& subsession, responseHandler* responseHandler,
                                     double start, double end, float scale,
                                     Authenticator* authenticator) {
  if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
...
unsigned WFDSink::sendTEARDOWNCommand(MediaSession& session, responseHandler* responseHandler, Authenticator* authenticator) {
  if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "TEARDOWN", responseHandler, &session));
}

unsigned WFDSink::sendTEARDOWNCommand(MediaSubsession& subsession, responseHandler* responseHandler, Authenticator* authenticator) {
  if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
  return sendRequest(new RequestRecord(++fCSeq, "TEARDOWN", responseHandler, NULL, &subsession));
}


Boolean WFDSink::changeResponseHandler(unsigned cseq, responseHandler* newResponseHandler) { 
...
Boolean WFDSink::lookupByName(UsageEnvironment& env,
  ...
  if (!medium->isSink()) {
    env.setResultMsg(instanceName, " is not a WFD client");
    return False;
  }

  resultClient = (WFDSink*)medium;
  return True;
}

Boolean WFDSink::parseWFDURL(UsageEnvironment& env, char const* url,
  ...
  do {
    ...
    char const* prefix = "wfd://";
    unsigned const prefixLength = 6;
...
void WFDSink::setUserAgentString(char const* userAgentName) {
...
unsigned WFDSink::responseBufferSize = 20000; // default value; you can reassign this in your application if you need to

WFDSink::WFDSink(UsageEnvironment& env, char const* rtspURL,
  ...
  sprintf(userAgentName, USERAGENT);
...
Boolean WFDSink::isWFDSink() const {
  return True;
}
...
int WFDSink::openConnection() {
  do {
    ...
    char const* urlSuffix;
    if (!parseWFDURL(envir(), fBaseURL, username, password, destAddress, urlPortNum, &urlSuffix)) break;
...
int WFDSink::connectToServer(int socketNum, portNumBits remotePortNum) {
...
char*
WFDSink::createAuthenticatorString(char const* cmd, char const* url) {
...
unsigned WFDSink::sendRequest(RequestRecord* request) {
  char* cmd = NULL;
  do {
    ...
    if (strcmp(request->commandName(), "DESCRIBE") == 0) {
      ...
      //envir() << "verbose: prefix=\""<<prefix<<"\", separator=\""<< separator<< "\", suffix=\""<<suffix<<"\"\n";
      char const* transportFmt;
      if (strcmp(subsession.protocolName(), "UDP") == 0) {
        suffix = "";
        transportFmt = "Transport: RTP/AVP/UDP%s%s%s=%d-%d\r\n";
      } else {
        transportFmt = "Transport: RTP/AVP/UDP%s%s%s=%d-%d\r\n";
      }
      ...
      if (fPresentationUrl == "") {
          ...
          cmdURL = new char[fPresentationUrl.length()];
          cmdURLWasAllocated = True;
          sprintf(cmdURL, "%s", fPresentationUrl.c_str());
      }
      ...
      NetAddress destAddress;
      portNumBits urlPortNum;
      if (!parseWFDURL(envir(), fBaseURL, username, password, destAddress, urlPortNum, (char const**)&cmdURL))
        break;
      if (cmdURL[0] == '\0')
        cmdURL = (char*)"/";
      delete[] username;
...
void WFDSink::handleRequestError(RequestRecord* request) {
...
void WFDSink::handleOPTIONSRequest(const ParsedMessage &msg)
{
    ...
    snprintf((char*)tmpBuf, sizeof tmpBuf,
        ...
        cseq);
    ssize_t sent = send(fOutputSocketNum, tmpBuf, strlen(tmpBuf), 0);
    envir() << ">>>>>>>>>>>>>>>>>>>>>>>>>>>\n";
    envir() << "Send response:" << (int)sent << " bytes:\n";
    envir() << tmpBuf << "\n\n";
    sendM2(NULL, NULL);
}
void WFDSink::handleGET_PARAMETERRequest(const ParsedMessage &msg)
{
    char tmpBuf[2*RTSP_PARAM_STRING_MAX];
    time_t now = time(NULL);
    struct tm *now2 = gmtime(&now);
    ...
    const char* body = 
        ...
        "wfd_client_rtp_ports: RTP/AVP/UDP;unicast 8888 0 mode=play\r\n"
        ;
...
void WFDSink::handleTriggerMethod(const ParsedMessage &msg)
{
    char tmpBuf[2*RTSP_PARAM_STRING_MAX];
    time_t now = time(NULL);
    struct tm *now2 = gmtime(&now);
...
void WFDSink::handleSET_PARAMETERRequest(const ParsedMessage &msg)
{
    char tmpBuf[2*RTSP_PARAM_STRING_MAX];
    time_t now = time(NULL);
    struct tm *now2 = gmtime(&now);
    ...
    if (strstr(msg.getContent(), "wfd_client_rtp_ports")) {
        const char* urlstart = NULL;
        if ((urlstart = strstr(msg.getContent(), "wfd_presentation_URL")) != NULL) {
            urlstart = strstr(urlstart, " ") + 1;
            envir() << "V: urlstart="<<urlstart;
            assert (urlstart != NULL);
            const char* urlend = strstr(urlstart, " ");
            envir() << "V: urlend="<<urlstart;
            assert (urlend != NULL);
            fPresentationUrl.assign(urlstart, urlend - urlstart);
            envir() << "V: presUrl="<<fPresentationUrl.c_str()<<"\n";
            assert (strstr(fPresentationUrl.c_str(), "\r\n") == NULL);
        }
        const char *sdp =
            "v=0\r\n"
            ...
            //"a=type:broadcast\r\n"
            "m=video 15550 RTP/AVP 32\r\n";
            //"c=IN IP4 239.255.42.42/127\r\n";
        setupSession(sdp);
    }
    if (strstr(msg.getContent(), "wfd_trigger_method")) {
        assert(fSession != NULL);
        assert(fSubsession != NULL);
        envir() << "YAO got wfd_trigger_method: " << msg.getContent() << "\n";
        if (strstr(msg.getContent(), "SETUP")) {
          envir() << "send setup command\n";
          sendSETUPCommand(*fSubsession,afterSETUP,False,False,False,NULL);
          envir() << "SETUP command send finished\n";
        }
        else if (strstr(msg.getContent(), "TEARDOWN")) {
            sendTEARDOWNCommand(*fSubsession, NULL, NULL);
        }
    }
    else {
        envir() << "YAO NO wfd_trigger_method: " << "\n";
...
void WFDSink::handleIncomingRequest() {
  ...
  ParsedMessage msg = ParsedMessage::Parse(fResponseBuffer,
    (size_t)fResponseBytesAlreadySeen, true, contentLength);
  if (msg.isValid()) {
    std::string reqField;
    int32_t statusCode;
    if (msg.getStatusCode(statusCode)) {
        envir() << "YAONEW Parsed response (" << (uint32_t)contentLength << " bytes)\n";
        //envir() << msg.getContent() << "\n";
    }
    else {
        envir() << "YAONEW Parsed request (" << (uint32_t)contentLength << " bytes)\n";;
        //envir() << msg.getContent() << "\n";
    }
    
  }
  else {
    envir() << "YAONEW Parsed failed\n";;
  }
  unsigned cl;
  if (!parseRTSPRequestString(fResponseBuffer, fResponseBytesAlreadySeen,
    ...
    if (strcmp(cmdName, "OPTIONS") == 0) {
        handleOPTIONSRequest(msg);
    }
    else if (strcmp(cmdName, "GET_PARAMETER") == 0) {
        handleGET_PARAMETERRequest(msg);
    }
    else if (strcmp(cmdName, "SET_PARAMETER") == 0) {
        //if (strstr(fResponseBuffer, "wfd_trigger_method:")) {
        //    handleTriggerMethod(msg);
        //}
        handleSET_PARAMETERRequest(msg);
    }
    else {
        char tmpBuf[2*RTSP_PARAM_STRING_MAX];
        time_t now = time(NULL);
...
Boolean WFDSink::checkForHeader(char const* line, char const* headerName, unsigned headerNameLength, char const*& headerParams) {
...
Boolean WFDSink::parseTransportParams(char const* paramsStr,
...
Boolean WFDSink::parseScaleParam(char const* paramStr, float& scale) {
  Locale l("C", Numeric);
  return sscanf(paramStr, "%f", &scale) == 1;
}

Boolean WFDSink::parseRTPInfoParams(char const*& paramsStr, u_int16_t& seqNum, u_int32_t& timestamp) {
...
Boolean WFDSink::handleOPTIONSResponse(MediaSubsession& subsession, char const* sessionParamsStr, char const* transportParamsStr)
{
    envir() << "my handle OPTIONS\n";
    return False;
}


void WFDSink::startPlay() {
    sendPLAYCommand(*fSubsession, NULL, 1, 1, 1, NULL);
}
      ...
      if (subsession.rtpSource() != NULL) {
        ...
        if (gs != NULL) {
          ...
          for (unsigned i = 0; i < numDummyPackets; ++i) {
            gs->output(envir(), 255, (unsigned char*)&dummy, sizeof dummy);
          }
        }
      }
    }
...
Boolean WFDSink::handlePLAYResponse(MediaSession& session, MediaSubsession& subsession,
...
Boolean WFDSink::handleTEARDOWNResponse(MediaSession& /*session*/, MediaSubsession& /*subsession*/) {
  // Because we don't expect to always get a response to "TEARDOWN", we don't need to do anything if we do get one:
  return True;
}

Boolean WFDSink::handleGET_PARAMETERResponse(char const* parameterName, char*& resultValueString)
{ return false; }
Boolean WFDSink::handleSET_PARAMETERResponse(char const* parameterName, char*& resultValueString)
{ return false; }

Boolean WFDSink::handleAuthenticationFailure(char const* paramsStr) {
...
Boolean WFDSink::resendCommand(RequestRecord* request) {
  if (fVerbosityLevel >= 1)
    envir() << "Resending...\n";
...
char const* WFDSink::sessionURL(MediaSession const& session) const {
...
void WFDSink::handleAlternativeRequestByte(void* rtspClient, u_int8_t requestByte) {
  ((WFDSink*)rtspClient)->handleAlternativeRequestByte1(requestByte);
}

void WFDSink::handleAlternativeRequestByte1(u_int8_t requestByte) {
  if (requestByte == 0xFF) {
...
void WFDSink::constructSubsessionURL(MediaSubsession const& subsession,
                    char const*& prefix,
...
Boolean WFDSink::setupHTTPTunneling1() {
...
void WFDSink::responseHandlerForHTTP_GET(WFDSink* rtspClient, int responseCode, char* responseString) {
  if (rtspClient != NULL) rtspClient->responseHandlerForHTTP_GET1(responseCode, responseString);
}

void WFDSink::responseHandlerForHTTP_GET1(int responseCode, char* responseString) {
  RequestRecord* request;
  do {
...
Boolean WFDSink::setupHTTPTunneling2() {
  fHTTPTunnelingConnectionIsPending = False;
...
void WFDSink::connectionHandler(void* instance, int /*mask*/) {
  WFDSink* client = (WFDSink*)instance;
  client->connectionHandler1();
}

void WFDSink::connectionHandler1() {
...
void WFDSink::incomingDataHandler(void* instance, int /*mask*/) {
  WFDSink* client = (WFDSink*)instance;
  client->incomingDataHandler1();
}

void WFDSink::incomingDataHandler1() {
  struct sockaddr_in dummy; // 'from' address - not used

  int bytesRead = readSocket(envir(),
                             fInputSocketNum,
...
void WFDSink::setupSession(const char* sdpDescription) {
  // Create a media session object from this SDP description:
  fSession = MediaSession::createNew(envir(), sdpDescription);
  envir() << "Start setup session\n";

  if (fSession == NULL) {
    envir() << "Failed to create a MediaSession object from the SDP description: \n";// << env->getResultMsg() << "\n";
...
void WFDSink::handleResponseBytes(int newBytesRead) {
  do {
    ...
    if (newBytesRead >= (int)fResponseBufferBytesLeft) {
      ...
      envir().setResultMsg("RTSP response was truncated. Increase \"WFDSink::responseBufferSize\"");
    }
  ...
  if (fVerbosityLevel >= 1 && newBytesRead > 1) 
    envir() << "<<<<<<<<<<<<<<<<<<<<<<<<<<<\nReceived " << newBytesRead << " new bytes of response data.\n";
  ...
  do {
    ...
    do {
      ...
      if (contentLength > numBodyBytes) {
        ...
        if (foundRequest != NULL)
            fRequestsAwaitingResponse.putAtHead(foundRequest);// put our request record back; we need it again
      ...
      if (foundRequest != NULL) {
        ...
        if (responseCode == 200) {
          ...
          } else if (strcmp(foundRequest->commandName(), "SET_PARAMETER") == 0) {
            if (!handleSET_PARAMETERResponse(foundRequest->contentStr(), bodyStart)) break;
          }
...
////////// WFDSink::RequestRecord implementation //////////

WFDSink::RequestRecord::RequestRecord(unsigned cseq, char const* commandName, responseHandler* handler,
...
WFDSink::RequestRecord::RequestRecord(unsigned cseq, responseHandler* handler,
                     char const* absStartTime, char const* absEndTime, float scale,
...
WFDSink::RequestRecord::~RequestRecord() {
...
////////// WFDSink::RequestQueue implementation //////////

WFDSink::RequestQueue::RequestQueue()
  : fHead(NULL), fTail(NULL) {
}

WFDSink::RequestQueue::RequestQueue(RequestQueue& origQueue)
  : fHead(NULL), fTail(NULL) {
  RequestRecord* request;
...
void WFDSink::RequestQueue::enqueue(RequestRecord* request) {
  if (fTail == NULL) {
    fHead = request;
  } else {
    fTail->next() = request;
...
WFDSink::RequestRecord* WFDSink::RequestQueue::dequeue() {
...
void WFDSink::RequestQueue::putAtHead(RequestRecord* request) {
  request->next() = fHead;
  fHead = request;
  if (fTail == NULL) {
    fTail = request;
...
WFDSink::RequestRecord* WFDSink::RequestQueue::findByCSeq(unsigned cseq) {
  RequestRecord* request;
